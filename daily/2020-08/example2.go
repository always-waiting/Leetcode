...
10. 钥匙和房间	--	https://leetcode-cn.com/problems/keys-and-rooms/
*/
...
func canVisitAllRooms(rooms [][]int) bool {
	keys := []int{}
	getKeyMap := map[int]bool{}
	keys = append(keys, rooms[0]...)
	for _, val := range rooms[0] {
		getKeyMap[val] = true
	}
	for len(keys) != 0 {
		key := keys[0]
		...
		getKeys := rooms[key]
		for _, val := range getKeys {
			if _, ok := getKeyMap[val]; !ok && val != 0 {
